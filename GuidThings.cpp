...
void GuidFromString(const wchar_t* TextGUID, GUID* Guid)
{
...
wstring NewStringFromGUID(GUID* Guid, const bool Hyphenate, const bool Brace, const bool Uppercase)
{
...
{
	GUID EmptyGUID{ 0 };
	wchar_t GUIDText[] = L"AB80BD77-B5EF-4701-B26E-D575BDBC5508";
	GuidFromString(GUIDText, &EmptyGUID);
	wstring OutGuid{ NewStringFromGUID(nullptr, true, true, true) };
	std::wcout << OutGuid.c_str();
	EmptyGuid(&EmptyGUID);
	OutGuid = NewStringFromGUID(&EmptyGUID, true, true, true);
	std::wcout << endl << OutGuid.c_str();
}
